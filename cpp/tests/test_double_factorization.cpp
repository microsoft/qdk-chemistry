...
TEST(DoubleFactorizerTest, MetaDataAndFactoryRegistration) {
  auto factorizer = DoubleFactorizerFactory::create("double_factorization");
...
TEST(DoubleFactorizerTest, RejectsInvalidInput) {
  ...
  auto truncating = DoubleFactorizerFactory::create("double_factorization");
...
TEST(DoubleFactorizerTest, PreservesOneBodyTermAndCoreEnergy) {
  ...
  auto factorized =
      DoubleFactorizerFactory::create("double_factorization")->run(hamiltonian);
...
TEST(DoubleFactorizerTest, TruncationDiscardsSmallFragments) {
  ...
  auto exact = DoubleFactorizerFactory::create("double_factorization");
  const auto num_ranks_exact =
      as_factorized(exact->run(hamiltonian)).get_num_ranks();

  auto truncated = DoubleFactorizerFactory::create("double_factorization");
...
TEST(DoubleFactorizerTest, StoredVectorsMatchDenseDecomposition) {
  ...
  auto from_stored =
      DoubleFactorizerFactory::create("double_factorization")->run(stored);
  auto from_dense =
      DoubleFactorizerFactory::create("double_factorization")->run(dense);
