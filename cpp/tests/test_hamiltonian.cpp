...
TEST_F(HamiltonianTest, Constructor) {
  ...
  Hamiltonian h(std::make_unique<CanonicalFourCenterHamiltonian>(
...
TEST_F(HamiltonianTest, MoveConstructor) {
  Hamiltonian h1(std::make_unique<CanonicalFourCenterHamiltonian>(
      one_body, two_body, orbitals, core_energy, inactive_fock));
...
TEST_F(HamiltonianTest, CopyConstructorAndAssignment) {
  ...
  Hamiltonian h3(std::make_unique<CanonicalFourCenterHamiltonian>(
      one_body, two_body, orbitals, core_energy, inactive_fock));
  h3 = h1;
  ...
  Hamiltonian h4(std::make_unique<CanonicalFourCenterHamiltonian>(
      one_body, two_body, orbitals, core_energy, inactive_fock));
...
TEST_F(HamiltonianTest, TwoBodyElementAccess) {
  ...
  Hamiltonian h_large(std::make_unique<CanonicalFourCenterHamiltonian>(
...
TEST_F(HamiltonianTest, ValidationTests) {
  ...
  EXPECT_THROW(
      Hamiltonian(std::make_unique<CanonicalFourCenterHamiltonian>(
  ...
  EXPECT_THROW(Hamiltonian(std::make_unique<CanonicalFourCenterHamiltonian>(
  ...
  EXPECT_NO_THROW(Hamiltonian(std::make_unique<CanonicalFourCenterHamiltonian>(
...
TEST_F(HamiltonianTest, UnrestrictedConstructor) {
  ...
  Hamiltonian h_unrestricted(std::make_unique<CanonicalFourCenterHamiltonian>(
...
TEST_F(HamiltonianTest, RestrictedVsUnrestrictedDetection) {
  ...
  Hamiltonian h_restricted(std::make_unique<CanonicalFourCenterHamiltonian>(
      one_body, two_body, orbitals, core_energy, inactive_fock));
...
TEST_F(HamiltonianTest, UnrestrictedJSONSerialization) {
  ...
  Hamiltonian h_orig(std::make_unique<CanonicalFourCenterHamiltonian>(
      one_body_alpha, one_body_beta, two_body_aaaa, two_body_aabb,
      two_body_bbbb, unrestricted_orbitals, core_energy, inactive_fock_alpha,
      inactive_fock_beta));
...
TEST_F(HamiltonianTest, FCIDUMPActiveSpaceConsistency) {
  ...
  Hamiltonian h_active_space(std::make_unique<CanonicalFourCenterHamiltonian>(
...
TEST_F(HamiltonianTest, IsValidComprehensive) {
  ...
  Hamiltonian h2(std::make_unique<CanonicalFourCenterHamiltonian>(
...
TEST_F(HamiltonianTest, GetContainerTypedAccess) {
  // Create a Hamiltonian with CanonicalFourCenterHamiltonian container
  Hamiltonian h(std::make_unique<CanonicalFourCenterHamiltonian>(
      one_body, two_body, orbitals, core_energy, inactive_fock));

  // Test successful typed container access
  EXPECT_NO_THROW({
    const auto& container = h.get_container<CanonicalFourCenterHamiltonian>();
    EXPECT_EQ(container.get_container_type(), "canonical_4_center");
  });

  // Verify has_container_type returns true for correct type
  EXPECT_TRUE(h.has_container_type<CanonicalFourCenterHamiltonian>());
