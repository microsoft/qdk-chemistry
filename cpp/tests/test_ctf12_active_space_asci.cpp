...
std::shared_ptr<data::Hamiltonian> restrict_dressed_virtual_space(
  ...
  Eigen::VectorXd two_body(static_cast<Eigen::Index>(
        ...
        for (std::size_t s = 0; s < n_active; ++s)
          two_body(static_cast<Eigen::Index>(
              ...
              hamiltonian->get_two_body_element(
                  source_positions[p], source_positions[q],
                  source_positions[r], source_positions[s]);
    }
...
std::shared_ptr<data::Wavefunction> closed_shell_determinant(
  ...
  auto determinant =
      data::Configuration::from_spin_half_string(occupations);
...
std::shared_ptr<data::Hamiltonian> make_ctf12_hamiltonian(
  ...
  constructor->settings().set(
      "frozen_core", static_cast<std::int64_t>(frozen_core));
  constructor->settings().set("cabs_basis", cabs_basis);
  ...
  return constructor->run(reference);
}
...
TEST(CtF12ActiveSpace, NeonAugCcPvdzAsci) {
  ...
  const ctf12::F12HartreeFockInput f12_input =
      ctf12::f12_input_from_wavefunction(
          *reference, 1.5, "aug-cc-pvdz-optri", 1);
  const double f12_hf_energy =
      hf_energy + ctf12::f12_hf_scf_energy(f12_input);
  const double f12_mp2_energy =
      hf_energy + ctf12::f12_mp2_energy(f12_input);

  auto constructor =
      algorithms::EffectiveHamiltonianConstructorFactory::create("qdk_ct_f12");
  constructor->settings().set("gamma", 1.5);
  constructor->settings().set("frozen_core", std::int64_t{1});
  constructor->settings().set("cabs_basis",
                              std::string("aug-cc-pvdz-optri"));
  constructor->settings().set("orbital_basis", std::string("relaxed"));
  ...
  EXPECT_EQ(active_hamiltonian->get_orbitals()
  ...
  const auto n_active_alpha =
      static_cast<unsigned int>(n_alpha - n_inactive);
  const auto n_active_beta =
      static_cast<unsigned int>(n_beta - n_inactive);
  ...
  EXPECT_EQ(n_active_beta, 4u);
    ...
    auto native_ansatz = std::make_shared<data::Ansatz>(
      *dressed_hamiltonian, *native_reference);
  ...
  auto mp2 = algorithms::DynamicalCorrelationCalculatorFactory::create(
    ...
    auto active_space_asci = make_asci(25);
  const auto [active_space_asci_energy, active_space_wavefunction] =
      active_space_asci->run(active_hamiltonian, n_active_alpha,
                             n_active_beta);
  ...
  EXPECT_GT(active_space_wavefunction->size(), 0u);
    EXPECT_LE(active_space_wavefunction->size(), 25u);
  ...
  EXPECT_EQ(active_space_wavefunction->get_active_num_electrons().second, 4u);
    ...
    auto full_valence_asci = make_asci(400);
    ...
    EXPECT_LE(full_valence_wavefunction->size(), 400u);
  ...
  std::cout << std::setprecision(16)
            ...
            << "Native CT-F12 MP2 energy (4-fold tensor): "
            << native_mp2_energy << " Hartree\n"
            ...
            << " orbitals, 8 correlated electrons, <=400 determinants) "
               "energy: "
...
TEST(CtF12AbsoluteEnergy, StretchedN2InternalAndEmissionBoundaries) {
  ...
  const ctf12::F12HartreeFockInput input =
    ...
    auto eri_ao = scf::geminal::four_center_coulomb(
    ...
    for (std::size_t j = 0; j < input.n_occupied; ++j) {
      original_determinant_energy +=
        2.0 * eri_mo[chemist_index(i, i, j, j)] -
    ...
    EXPECT_NEAR(original_determinant_energy, hf_energy, 1e-8);
      ...
      for (std::size_t q = 0; q < n_mo; ++q)
        for (std::size_t i = 0; i < input.n_occupied; ++i)
        reference_fock(p, q) +=
          2.0 * eri_mo[chemist_index(p, q, i, i)] -
      ...
      const double max_fock_off_diagonal =
        off_diagonal.cwiseAbs().maxCoeff();
  ...
  EXPECT_NEAR(dressed.e_hf + nuclear_repulsion, hf_energy, 1e-8);
    EXPECT_NEAR(dressed.e_hf + nuclear_repulsion,
          original_determinant_energy, 1e-8);

  auto index = [&](std::size_t p, std::size_t q, std::size_t r,
                   std::size_t s) {
  ...
  for (std::size_t i = 0; i < dressed.n_occupied; ++i) {
    ...
    for (std::size_t j = 0; j < dressed.n_occupied; ++j) {
      emitted_determinant_energy +=
          2.0 * dressed.two_body[index(i, i, j, j)] -
          dressed.two_body[index(i, j, j, i)];
    }
  }
  EXPECT_NEAR(emitted_determinant_energy,
              dressed.e_f12hf + nuclear_repulsion, 1e-8);
  EXPECT_NEAR(emitted_determinant_energy,
  ...
  TEST(CtF12AbsoluteEnergy, StretchedN2HamiltonianMatchesHfAndMp2Apis) {
    ...
    const ctf12::F12HartreeFockInput input =
      ctf12::f12_input_from_wavefunction(*reference, gamma, cabs_basis,
                       frozen_core);
    ...
    const auto hamiltonian = make_ctf12_hamiltonian(
      reference, gamma, frozen_core, cabs_basis,
    ...
    std::cout << std::setprecision(16)
              << "Canonical HF total: " << hf_energy << " Hartree\n"
              << "CT-F12 HF API total: " << expected_f12_hf_energy
              << " Hartree\n"
              << "Emitted-H determinant: " << emitted_f12_hf_energy
              << " Hartree\n"
              << "CT-F12 MP2 API total: " << expected_f12_mp2_energy
              << " Hartree\n"
              << "MP2 over emitted H: " << emitted_f12_mp2_energy
              << " Hartree\n";
...
TEST(CtF12ActiveSpace, StretchedN2CcPvdzAsci) {
  ...
  constexpr std::int64_t max_asci_determinants = 400;
  const std::string orbital_basis = "cc-pvdz-f12";
  const std::string cabs_basis = "cc-pvdz-f12-optri";

  const auto structure = testing::create_stretched_n2_structure(bond_length_angstrom);
  auto scf_solver = algorithms::ScfSolverFactory::create("qdk");
  const auto [hf_energy, reference] = scf_solver->run(structure, 0, 1, orbital_basis);
  const auto [n_alpha, n_beta] = reference->get_total_num_electrons();
  ...
  const auto native_hamiltonian = make_ctf12_hamiltonian(reference, gamma, frozen_core, cabs_basis,
      /*symmetrize_two_body=*/false);
  const auto symmetrized_hamiltonian = make_ctf12_hamiltonian(reference, gamma, frozen_core, cabs_basis,
      /*symmetrize_two_body=*/true);

  const double native_f12_hf_energy = closed_shell_determinant_energy(native_hamiltonian, n_active_alpha);
  const double native_f12_mp2_energy = mp2_energy(native_hamiltonian, n_active_alpha);
  const double symmetrized_f12_hf_energy = closed_shell_determinant_energy(symmetrized_hamiltonian, n_active_alpha);
  const double symmetrized_f12_mp2_energy = mp2_energy(symmetrized_hamiltonian, n_active_alpha);

    const auto full_valence_indices = native_hamiltonian->get_orbitals()->get_active_space_indices().first;
  ...
  const std::vector<std::size_t> cas_indices(
    ...
    const auto cas_hamiltonian = restrict_dressed_virtual_space(native_hamiltonian, cas_indices);
  const double cas_hf_energy = closed_shell_determinant_energy(cas_hamiltonian, n_active_alpha);
    EXPECT_NEAR(cas_hf_energy, native_f12_hf_energy, 1e-12);

  auto cas_asci = make_asci(max_asci_determinants);
  const auto [cas_asci_energy, cas_wavefunction] = cas_asci->run(cas_hamiltonian, n_active_alpha, n_active_beta);
  ASSERT_NE(cas_wavefunction, nullptr);
  EXPECT_TRUE(std::isfinite(cas_asci_energy));
  EXPECT_LE(cas_asci_energy, cas_hf_energy + 1e-10);
  EXPECT_LE(cas_wavefunction->size(), static_cast<std::size_t>(max_asci_determinants));
  ...
  auto full_valence_asci = make_asci(max_asci_determinants);
  const auto [full_valence_asci_energy, full_valence_wavefunction] =
      full_valence_asci->run(native_hamiltonian, n_active_alpha,
                             n_active_beta);
  ASSERT_NE(full_valence_wavefunction, nullptr);
  EXPECT_TRUE(std::isfinite(full_valence_asci_energy));
    EXPECT_LE(full_valence_asci_energy, native_f12_hf_energy + 1e-10);
  EXPECT_LE(full_valence_wavefunction->size(),
            static_cast<std::size_t>(max_asci_determinants));
  ...
  std::cout << std::setprecision(16)
            ...
            << "Electron partition: 14 total = 4 frozen-core + 10 "
               "correlated\n"
            << "HF energy: " << hf_energy << " Hartree\n"
            << "Native F12-HF energy (4-fold tensor): "
            << native_f12_hf_energy << " Hartree\n"
            ...
            << "Native F12-ASCI (CAS(10e,8o), <=400 determinants): "
            << cas_asci_energy << " Hartree\n"
            << "Native F12-ASCI (full frozen-core valence space, "
            << full_valence_indices.size()
            << " orbitals, 10 correlated electrons, <=400 determinants): "
            << full_valence_asci_energy << " Hartree\n";
}
