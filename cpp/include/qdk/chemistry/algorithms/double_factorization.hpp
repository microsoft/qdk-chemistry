...
struct TwoBodyFragment {
  ...
  /// in the convention of whichever tensor the caller passed in (Patel 2024).
...
///        magnitude falls below this threshold are dropped. 0.0 performs no
///        truncation, making the factorization exact.
...
///       zero. At threshold 0.0 those null fragments are still returned; they
///       contribute nothing but do inflate the fragment count.
 ...
 *
 * Default settings:
 * - truncation_threshold: 0.0 - no truncation, so the factorization is exact.
 *
 ...
 public:
  ...
  DoubleFactorizerSettings() {
    set_default<double>(
        ...
        "is below this threshold. Must be non-negative; 0.0 performs no "
        "truncation, so the factorization is exact.",
