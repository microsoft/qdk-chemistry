...
std::vector<std::size_t> validate_basis_compatibility(
    const CholeskyHamiltonianContainer& source, const Orbitals& target_orbitals,
    double tolerance) {
  ...
  require(source_orbitals->has_basis_set() == target_orbitals.has_basis_set() &&
...
Eigen::MatrixXd recover_active_rotation(
    const Orbitals& source_orbitals, const Orbitals& target_orbitals,
    const std::vector<std::size_t>& source_indices, double tolerance) {
  ...
  const auto& stored_source_overlap = source_orbitals.get_overlap_matrix();
  const auto& stored_target_overlap = target_orbitals.get_overlap_matrix();
  const Eigen::MatrixXd source_metric =
      0.5 * (stored_source_overlap + stored_source_overlap.transpose());
  {
    const Eigen::MatrixXd metric_difference =
        0.5 * (stored_target_overlap + stored_target_overlap.transpose()) -
        source_metric;
    require_close(target_active_coefficients.transpose() * metric_difference *
                      target_active_coefficients,
                  Eigen::MatrixXd::Zero(nactive, nactive), tolerance,
                  "Target active-orbital overlap in the target AO metric");
  }
  const auto& overlap = source_metric;
  ...
  if (numerical_null_mode_coefficients) {
    require_close(numerical_null_mode_coefficients->first * recovered_rotation,
...
std::shared_ptr<data::Hamiltonian> QdkHamiltonianBasisTransformer::_run_impl(
  ...
  const auto source_indices =
      validate_basis_compatibility(source, *target_orbitals, tolerance);
  const Eigen::MatrixXd rotation = recover_active_rotation(
      *source.get_orbitals(), *target_orbitals, source_indices, tolerance);
  const Eigen::Index nactive = source_indices.size();
