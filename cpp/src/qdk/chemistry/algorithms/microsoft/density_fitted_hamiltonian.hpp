 ...
 * (ij|kl), it computes raw three-center integrals E_(ij,P) = (ij|P), then
 * folds in the auxiliary Coulomb metric M = L L^T to store the
 * metric-orthonormalized factors B = E L^(-T).
 *
 ...
 *
 * @note The auxiliary-basis collection passed to run() must contain an exact
 * RIFit association for the same molecular structure as the primary basis.
 *
