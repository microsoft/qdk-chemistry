...
void compute_trial_fock(const std::shared_ptr<qcs::ERI> eri,
  ...
  const bool unrestricted = (static_cast<size_t>(ground_density.rows()) == 2 * num_atomic_orbitals);
 ...
 private:
  ...
  void apply_stability_operator(const double* X_ptr, double* Y_ptr,
    ...
    const bool unrestricted =
        (static_cast<size_t>(ground_density_.rows()) == 2 * num_atomic_orbitals);
...
StabilityChecker::_run_impl(
  ...
  // configured here
  scf_config->scf_orbital_type = unrestricted
  ...
  if (check_external) {
    ...
    if (method != "HF") {
      ...
      scf_config_external->scf_orbital_type = qcs::SCFOrbitalType::Unrestricted;
