...
template <size_t N>
asci_helper::return_type asci_helper::impl(const data::Hamiltonian& hamiltonian,
  ...
  size_t fci_dimension = qdk::chemistry::utils::microsoft::binomial_coefficient(
  ...
  if (asci_settings.ntdets_max > fci_dimension) {
    ...
    QDK_LOGGER().debug(
        "Completed MACIS ASCI growth phase with {} determinants.", dets.size());
