...
nlohmann::ordered_json convert_to_json(
  ...
  // Retain the element map for backward compatibility when the ECP treatment
  // is uniform for every atom of an element.
  ...
  for (size_t i = 0; i < ecp_electrons.size(); ++i) {
    ...
    int ncore = static_cast<int>(ecp_electrons[i]);
