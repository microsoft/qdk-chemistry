...
void ERI_DF::build_JK(const double* P, double* J, double* K, double alpha,
  ...
  if (gpu()) {
    ...
    blas::gemv("T", num_atomic_orbitals2, naux_loc, 1.0, h_eri_.get(),
               num_atomic_orbitals2, P_use, 1, 0.0, X_loc, 1);
  }
  ...
  if (gpu()) {
    ...
    blas::gemv("N", num_atomic_orbitals2, naux_loc, 1.0, h_eri_.get(),
               num_atomic_orbitals2, X_loc, 1, 0.0, J, 1);
  }
