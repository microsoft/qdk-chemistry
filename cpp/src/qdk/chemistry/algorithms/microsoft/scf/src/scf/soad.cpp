...
/*
  For each element, the array contains the number of atomic orbitals for each
  shell
*/
uint64_t element_ao_count[][11] = {
    {0},                                // placeholder
    {1},                                // Z: 1
    {1},                                // Z: 2
    {1, 1},                             // Z: 3
...
uint64_t element_electron_distribution[][11] = {
    {0},                                  // placeholder
    {1},                                  // Z: 1
    {2},                                  // Z: 2
    {2, 1},                               // Z: 3
...
/* Initializing the density matrix P with Superposition-Of-Atomic-Densities
 * guess */
void soad_initialize_density_matrix(double *density_matrix, uint64_t N,
  ...
  for (uint64_t i = 0; i < n_atoms; ++i) {
    ...
    uint64_t shell_count = element_shell_count[Z];
    uint64_t *ao_counts = element_ao_count[Z];
    uint64_t *electrons_per_shell = element_electron_distribution[Z];
    for (uint64_t j = 0; j < shell_count; ++j) {
      uint64_t ao_count = ao_counts[j];
      double electrons_per_ao = 0.5 * electrons_per_shell[j] / ao_count;
      for (uint64_t k = 0; k < ao_count; ++k) {
