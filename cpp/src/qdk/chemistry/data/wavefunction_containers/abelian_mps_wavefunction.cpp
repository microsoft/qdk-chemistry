...
void validate_abelian_bond_symmetry(const SymmetryProduct& symmetry) {
  if (!symmetry.has_axis(AxisName::ParticleNumber)) {
    throw std::invalid_argument(
        "AbelianMPSContainer requires particle-number-blocked bond "
        "sectors. Use particle_number(...) when constructing the bond "
        "SymmetryProduct.");
  }
}
...
void AbelianMPSSite::_validate() const {
...
bool AbelianMPSSite::is_complex() const {
  return _physical_slices.front()->index() == 1;
}

AbelianMPSSite::DenseMatrixVariant AbelianMPSSite::to_dense() const {
  return std::visit(
