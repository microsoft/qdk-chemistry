...
// const Eigen::VectorXd& HamiltonianContainer::get_two_body_integrals() const {
//   if (!has_two_body_integrals()) {
//     throw std::runtime_error("Two-body integrals are not set");
//   }
//   return *std::get<0>(_two_body_integrals);  // Return alpha-alpha integrals
//   for
...
bool HamiltonianContainer::has_orbitals() const { return _orbitals != nullptr; }

double HamiltonianContainer::get_core_energy() const { return _core_energy; }

HamiltonianType HamiltonianContainer::get_type() const { return _type; }

bool HamiltonianContainer::is_hermitian() const {
  return _type == HamiltonianType::Hermitian;
}

// bool HamiltonianContainer::is_restricted() const {
...
bool HamiltonianContainer::is_unrestricted() const { return !is_restricted(); }

// bool HamiltonianContainer::_is_valid() const {
...
void HamiltonianContainer::validate_integral_dimensions() const {
  ...
  //       expected_size) {
  //     throw std::invalid_argument("Beta-beta two-body integrals size
  //     mismatch");
  //   }
  // }
}
...
// size_t HamiltonianContainer::get_two_body_index(size_t i, size_t j, size_t k,
//                                        size_t l) const {
//   size_t norb = _orbitals->get_active_space_indices().first.size();
...
// std::tuple<std::shared_ptr<Eigen::VectorXd>,
...
//     // Load Hamiltonian type (default to Hermitian for backward
//     compatibility) HamiltonianType type = HamiltonianType::Hermitian; if
//     (j.contains("type")) {
...
//       throw std::runtime_error("Hamiltonian JSON must include orbitals
//       data");
//     }
...
//         // This is always restricted format
//         auto two_body_vec =
//         j["two_body_integrals"].get<std::vector<double>>();
...
//         for (int i = 0; i < rows; ++i) {
//           for (int j_idx = 0; j_idx < cols; ++j_idx) {
//             inactive_fock_alpha(i, j_idx) =
//             inactive_fock_alpha_vec[i][j_idx];
//           }
//         }
...
//           int cols_beta = rows_beta > 0 ? inactive_fock_beta_vec[0].size() :
//           0; inactive_fock_beta.resize(rows_beta, cols_beta); for (int i = 0;
//           i < rows_beta; ++i) {
//             for (int j_idx = 0; j_idx < cols_beta; ++j_idx) {
//               inactive_fock_beta(i, j_idx) =
//               inactive_fock_beta_vec[i][j_idx];
//             }
//           }
//         }
...
//       }
//       // Core energy should be explicitly set when there are inactive
//       orbitals if (!j.contains("core_energy")) {
//         auto inactive_indices = orbitals->get_inactive_space_indices();
//         size_t total_inactive =
//             inactive_indices.first.size() + inactive_indices.second.size();
//         throw std::runtime_error(
...
//     if (is_restricted_data) {
//       // Use restricted constructor - it will create shared pointers
//       internally
...
//         metadata_group.createAttribute("type", type_string_type,
//         scalar_space);
...
//     if (!orbitals) {
//       throw std::runtime_error("Hamiltonian HDF5 must include orbitals
//       data");
//     }
...
//       one_body_beta = load_matrix_from_group(group,
//       "one_body_integrals_beta");
//     }

//     // Load two-body integrals
//     if (dataset_exists_in_group(group, "two_body_integrals_aaaa")) {
//       two_body_aaaa = load_vector_from_group(group,
//       "two_body_integrals_aaaa");
//     }
...
//     // Create and return appropriate Hamiltonian using the correct
//     constructor if (is_restricted_data) {
//       // Use restricted constructor - it will create shared pointers
//       internally return std::make_shared<Hamiltonian>(one_body_alpha,
//       two_body_aaaa,
//                                            orbitals, core_energy,
//                                            inactive_fock_alpha, type);
//     } else {
...
// void Hamiltonian::_to_fcidump_file(const std::string& filename, size_t
// nalpha,
//                                    size_t nbeta) const {
//   std::ofstream file(filename);
//   if (!file.is_open()) {
...
//               .first.size();  // TODO: Assumes the same indices for
//               alpha/beta
//     } else {
