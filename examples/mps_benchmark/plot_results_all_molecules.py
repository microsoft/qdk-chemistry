...
Reads data from mps_sossa_resource_estimation_*.json files include:
...
for json_path in JSON_FILES:
    ...
    mol_keys = [k for k in data.keys() if k != "parameters"]
...
for col, md in enumerate(mol_data):
    ...
    # --- Top row: Runtime ---
    ax_rt.plot(md["mps_chis"], md["mps_rt"], marker="o", linestyle="-",
               color="0.4", markersize=7, linewidth=2, markerfacecolor="0.5",
               markeredgecolor="black", label="MPS State Prep")
    if md["qpe_chis"]:
        ax_rt.plot(md["qpe_chis"], md["qpe_rt"], marker="s", linestyle="--",
                   color="black", markersize=7, linewidth=2, markerfacecolor="none",
                   markeredgecolor="black", markeredgewidth=1.5, label="QPE-MPS")
    if md["qpe_hf_runtime"]:
        ax_rt.axhline(md["qpe_hf_runtime"], color="black", linestyle=":",
                      linewidth=2, label=f"QPE-HF")
    ...
    ax_tf.plot(md["mps_chis"], md["mps_tf"], marker="o", linestyle="-",
               color="0.4", markersize=7, linewidth=2, markerfacecolor="0.5",
               markeredgecolor="black", label="MPS State Prep")
    if md["qpe_chis"]:
        ax_tf.plot(md["qpe_chis"], md["qpe_tf"], marker="s", linestyle="--",
                   color="black", markersize=7, linewidth=2, markerfacecolor="none",
                   markeredgecolor="black", markeredgewidth=1.5, label="QPE-MPS")
    if md["qpe_hf_toffoli"]:
        ax_tf.axhline(md["qpe_hf_toffoli"], color="black", linestyle=":",
                      linewidth=2, label=f"QPE-HF")
    ...
    ax_qb.plot(md["mps_chis"], md["mps_qb"], marker="o", linestyle="-",
               color="0.4", markersize=7, linewidth=2, markerfacecolor="0.5",
               markeredgecolor="black", label="MPS State Prep")
    if md["qpe_chis"]:
        ax_qb.plot(md["qpe_chis"], md["qpe_qb"], marker="s", linestyle="--",
                   color="black", markersize=7, linewidth=2, markerfacecolor="none",
                   markeredgecolor="black", markeredgewidth=1.5, label="QPE-MPS")
    if md["qpe_hf_qubits"]:
        ax_qb.axhline(md["qpe_hf_qubits"], color="black", linestyle=":",
                      linewidth=2, label=f"QPE-HF")
