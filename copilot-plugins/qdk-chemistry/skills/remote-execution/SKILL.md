...
Use local execution conservatively: default to one local worker/job at a time,
inspect available CPU, memory, accelerator, and scratch-space limits before
sizing the calculation, and avoid internal parallelism that oversubscribes the
machine. If the local resource budget is unknown or insufficient for the
requested problem size, prefer an SSH/HPC backend or reduce the calculation size
instead of launching competing local jobs. Increase local concurrency only after
an explicit policy decision confirms the machine can handle it.
...
The MCP server exposes remote execution through every `run_*` chemistry tool plus 4 job-management tools. There is **no** standalone `submit_remote_job` tool. To run remotely, call the same `run_*` tool you would use locally and add remote parameters.
...
- **Nested settings**: Remote manifests preserve `AlgorithmRef` settings recursively. Use the same nested algorithm dictionaries accepted by local MCP calls; no flattening or local fallback is required.
...
Treat the first basic remote failure as recoverable and make at least one
diagnosed retry before declaring the calculation inconclusive:

1. Call `check_remote_job` and inspect the status, logs, and exact error.
2. If a succeeded job fails during retrieval, retry `retrieve_remote_results`
    once with the same job ID. If the artifact remains unavailable, resubmit the
    producing `run_*` step once.
3. If execution failed for a transient service, transport, or infrastructure
    reason, resubmit once with unchanged scientific settings.
4. If the error is deterministic, correct it before retrying. For example,
    regenerate an open-shell ASCI/AutoCAS input with
    `settings={"method": "hf", "scf_type": "restricted"}`, or verify the
    charge/multiplicity combination after an invalid spin/charge error.
5. Do not repeat an unchanged job for authentication, authorization, quota, or
    unsupported-method errors. Report the blocker and the attempted recovery.

Record both job IDs, all error messages, and every changed parameter. A retry
must not silently change the requested charge, multiplicity, basis, active
space, or endpoint.
