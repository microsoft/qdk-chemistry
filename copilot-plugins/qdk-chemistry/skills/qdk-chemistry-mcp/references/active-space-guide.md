...
1. **Generate compatible HF orbitals** — ASCI, valence selection, and AutoCAS require one shared set of spatial orbitals. For an open-shell state (`spin_multiplicity > 1`), call `run_scf` with `settings={"method": "hf", "scf_type": "restricted"}` to generate an ROHF reference. Do not use the default `scf_type="auto"` or an unrestricted HF/DFT reference for this path: open-shell `auto` produces separate alpha and beta orbitals that the downstream ASCI/AutoCAS workflow cannot consume. Closed-shell states use the corresponding RHF reference.

2. **Get an initial active space with RDMs** — AutoCAS needs reduced density matrices. You can't feed it a bare SCF wavefunction. Two ways to get RDMs:
...
3. **Run SCI with all three flags:**
...
4. **Run AutoCAS:** `run_active_space_selector` with `algorithm_name="qdk_autocas_eos"` on the SCI result. This reads the orbital entropies and identifies the strongly correlated subset.

5. **Visualize and verify:** Call `visualize_orbital_entanglement` on the full wavefunction, with the AutoCAS-selected indices as `selected_indices`, to confirm the selection makes chemical sense. Then `visualize_orbitals` to show the selected orbital shapes.

6. **Proceed with the refined space** — rebuild the Hamiltonian on the compressed active space.

## Empty AutoCAS Selections

An exact AutoCAS/AutoCAS-EOS result with zero active electrons is a valid formal
resource lower bound. For example, if the selector returns `CAS(0e,2o)`, record
the selected orbitals and selector diagnostics, classify the quantum stage as
requiring no quantum computation, and report zero logical and physical quantum
resources for that stage. Keep classical preprocessing costs separate.

Add a prominent warning that an empty active space is only a formal lower bound
and does not independently establish energy accuracy. Do not alter entropy
thresholds or substitute another selector merely to force a nonempty active
space unless the user or campaign explicitly requests a sensitivity study.
