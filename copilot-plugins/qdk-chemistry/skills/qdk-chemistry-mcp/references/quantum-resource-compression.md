...
- For a full-valence upper-bound QRE workflow, feed exactly the top 10 ASCI
  determinants into `run_projected_multi_configuration_calculation` unless the
  campaign explicitly overrides the default. Other workflows may approve a
  different count.
- Record cumulative squared amplitude or overlap and a truncation warning; do
  not assume that the top 10 retain a fixed percentage for every molecule.
...
- Without approved energy bounds, derive `evolution_time` from the mapped
	qubit-Hamiltonian coefficient 1-norm:
	`lambda_1 = sum(abs(coefficients))`, `t = π / lambda_1`.
