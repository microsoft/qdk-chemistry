...
- **Evolution time:** Set this explicitly from target energy bounds. Without
	approved bounds, use the mapped qubit-Hamiltonian coefficient 1-norm
	`lambda_1 = sum(abs(coefficients))`, with
	`energy_window = 2 * lambda_1` and `evolution_time = π / lambda_1`; there is
	no public `compute_evolution_time()` helper
