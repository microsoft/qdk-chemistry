...
- **Circuit resource analysis** → build QPE circuit components, use `get_circuit_stats` for logical metrics, and run `run_resource_estimation` for the physical Pareto frontier
...
- **QDK results are the only source of QRE numbers** — never infer, extrapolate,
  ...
  unavailable
- **Validate the endpoint** — mapping statistics, logical circuit statistics,
  state-preparation estimates, and QPE physical-resource estimates are distinct
