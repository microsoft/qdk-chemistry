...
Start with RHF and check stability. UHF can diagnose spin-symmetry breaking, but its separate alpha and beta orbitals cannot feed the valence → ASCI → AutoCAS sequence below. Keep a restricted HF reference for that path; for an open-shell multiplicity, request `scf_type="restricted"` to obtain ROHF orbitals.
...
12. run_phase_estimation(settings={"qpe_circuit_builder": {"num_bits": <derived>,
     "unitary_builder": {"algorithm_name": "trotter", "time": <derived>},
...
- Would use ROHF for any open-shell orbitals passed to valence selection, ASCI, or AutoCAS; UHF is only suitable for a separate path that accepts unrestricted orbitals
