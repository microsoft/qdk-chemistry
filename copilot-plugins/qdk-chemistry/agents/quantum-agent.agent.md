...
The **chemist** agent calls `get_circuit_stats` and `run_resource_estimation` after each circuit-producing step and includes the logical metrics and physical Pareto points in its report. Use this evidence in the final report delegation to the **reporter**.
