...
def create_measurement_circuits(circuit: Circuit, grouped_hamiltonians: list[QubitHamiltonian]) -> list[Circuit]:
    ...
    """
    meas_circuits = []
    base_circuit = qasm3.loads(circuit.get_circuit_qasm())
    ...
    for hamiltonian in grouped_hamiltonians:
        ...
        meas_circuits.append(Circuit(circuit_qasm=qasm3.dumps(full_circ)))
...
class EnergyEstimator(Algorithm):
    ...
    def run(
        ...
        return super().run(circuit, qubit_hamiltonians, total_shots, classical_coeffs)
