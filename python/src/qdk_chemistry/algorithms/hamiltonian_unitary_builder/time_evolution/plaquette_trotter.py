...
class PlaquetteTrotter(Trotter):
    ...
    def _resolve_num_divisions(self, qubit_hamiltonian: QubitOperator, time: float) -> int:
        ...
        interaction = 0.0
        for mapping, coeff in self._real_terms(qubit_hamiltonian, 1e-12):
            positions = sorted(mapping)
            if len(positions) == 2 and all(mapping[position] == "Z" for position in positions):
                low, high = positions
    ...
    def _decompose_trotter_step(
        ...
        Raises:
            ValueError: If the lattice shape is unset or inconsistent with the Hamiltonian, or hopping is not uniform.
    ...
    @classmethod
    def _real_terms(cls, qubit_hamiltonian: QubitOperator, atol: float) -> Iterator[tuple[dict[int, str], float]]:
        """Read PR728 sparse factors without materializing full-register labels."""
        labels = qubit_hamiltonian.pauli_strings
        for index, coefficient in enumerate(qubit_hamiltonian.coefficients):
            real = complex(coefficient).real
            if abs(real) > atol:
                mapping = (
                    dict(labels.factors(index))
                    if isinstance(labels, SparsePauliTerms)
                    else cls._pauli_label_to_map(labels[index])
                )
                yield mapping, real

    def _split_hopping(
        self, qubit_hamiltonian: QubitOperator, num_sites: int, atol: float
    ) -> tuple[float, list[ExponentiatedPauliTerm], set[frozenset[int]]]:
        ...
        Returns:
            ``(hopping, diagonal, bonds)`` with amplitude ``t``, unit-time interaction terms, and site pairs.

        Raises:
            ValueError: If hopping is absent, nonuniform, connects the spin blocks, or the spin sectors disagree.

        """
        hopping_terms: dict[int, dict[frozenset[int], dict[str, float]]] = {0: {}, 1: {}}
        diagonal: list[ExponentiatedPauliTerm] = []
        for mapping, coeff in self._real_terms(qubit_hamiltonian, atol):
            ...
            if len(axes) != 2:
                raise ValueError(
                    f"The term {mapping!r} is not a Jordan-Wigner hopping string: expected exactly two X/Y endpoints."
                )

            first, second = sorted(axes)
            if (first < num_sites) != (second < num_sites):
                raise ValueError(
                    f"The term {mapping!r} hops between the spin-up and spin-down blocks. "
            ...
            if (
                ...
                raise ValueError(f"The term {mapping!r} is not a canonical Jordan-Wigner XX/YY hopping string.")
