...
class DynamicModeDecompositionSettings(PhaseEstimationSettings):
    ...
    def __init__(self):
        ...
        self._set_default("max_rank_k", "int", 100, "Column number limit of Hankel matrix X")
...
class DynamicModeDecomposition(PhaseEstimation):
    ...
    def _run_impl(self,
        ...
        *,
        ...
        for i in range (0, initial_number_measurement - 1):
        ...
        while abs(phase_fraction - last_phase_fraction) > 1e-6:
            if rank_k == max_rank_k + 1:
                Logger.warning(f"reached the limit of rank_k {max_rank_k}! ODMD iteration will stop here!")
                break
            ...
            executor_data = circuit_executor.run(circuit_real, shots=shots, noise=noise)
            bitstring_result = executor_data.bitstring_counts # a dictionary
            Logger.info(f"measured real part of {observe_index + 1} observable, {bitstring_result.get("0", 0)} zeros, {bitstring_result.get("1", 0)} ones")
            observable_real = (bitstring_result.get("0", 0) - bitstring_result.get("1", 0)) / shots
            ...
            X = np.column_stack((X, X_prime[:, rank_k - 2]))
            X_prime = np.column_stack((X_prime, observable_array[rank_k : rank_k + vector_dim]))
