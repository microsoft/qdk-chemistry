...
class QdkQreV1Settings(Settings):
    ...
    def __init__(self) -> None:
        ...
        super().__init__()
        self._set_default("error_budget", "double", 0.001, "Total error budget for the estimation")
...
class QdkQreV1(ResourceEstimator):
    ...
    def _run_impl(
        ...
        elif circuit.qir is not None:
            # Circuit was loaded from disk with only QIR (factory data is
            # lost during serialization).  Convert QIR → QASM via Qiskit
            # so the Q# OpenQASM estimator can process it.
            qasm_str = circuit.get_qasm()
            result = qsharp.openqasm.estimate(qasm_str, params)
        else:
            raise RuntimeError(
                "Cannot estimate resources: no Q# factory data, QASM, or QIR representation is available."
            )
