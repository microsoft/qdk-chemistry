...
def orbitals_to_scf(
    ...
    Args:
        ...
        force_restricted: If True, forces the creation of a restricted SCF object (RHF or ROHF).

            Applies even if the input orbitals are unrestricted. Default is False.
...
def hamiltonian_to_scf(hamiltonian: Hamiltonian, alpha_occ: np.ndarray, beta_occ: np.ndarray) -> pyscf.scf.RHF:
    ...
    Raises:
        ValueError: If the Hamiltonian uses unsupported features.

            Examples are: unrestricted orbitals, open-shell systems, or active spaces.
...
def rotate_orbitals(
    ...
    >>> eigenvals, eigenvecs = rhf_internal(mf)
    >>> if eigenvals[0] < -1e-5:  # Unstable
    ...     rotated_orbitals = rotate_orbitals(orbitals, eigenvecs[:, 0], num_alpha, num_beta)

    Notes
    -----
    This function assumes aufbau filling for the occupation numbers.
    For UHF wavefunctions, the rotation vector should contain both alpha
    and beta rotations concatenated.

    """
    # Create occupation arrays (assuming aufbau filling)
    num_molecular_orbitals = orbitals.get_num_molecular_orbitals()
    occ_alpha = np.array([1.0 if i < num_alpha else 0.0 for i in range(num_molecular_orbitals)])
    occ_beta = np.array([1.0 if i < num_beta else 0.0 for i in range(num_molecular_orbitals)])

    # Convert to PySCF SCF object to get proper occupation format
    ...
    if orbitals.is_restricted():
        if restricted_external:
            ...
            rotated_coeff_alpha = _rotate_mo(mf.mo_coeff, mf.mo_occ, rotation_vector)
            rotated_coeff_beta = mf.mo_coeff

            rotated_orbitals = Orbitals(
                coefficients_alpha=rotated_coeff_alpha,
                coefficients_beta=rotated_coeff_beta,
                energies_alpha=None,  # Energies are invalidated by rotation
                energies_beta=None,  # Energies are invalidated by rotation
            ...
            rotated_coeff = _rotate_mo(mf.mo_coeff, mf.mo_occ, rotation_vector)

            rotated_orbitals = Orbitals(
                coefficients=rotated_coeff,
                energies=None,  # Energies are invalidated by rotation
                ao_overlap=orbitals.get_overlap_matrix() if orbitals.has_overlap_matrix() else None,
                basis_set=orbitals.get_basis_set(),
            )
    else:
        # Unrestricted case - separate alpha and beta rotations
        # For UHF, rotation_vector typically contains alpha rotations first, then beta
        num_alpha_virtual_orbitals = num_molecular_orbitals - num_alpha
        alpha_size = num_alpha * num_alpha_virtual_orbitals

        rotation_alpha = rotation_vector[:alpha_size]
        rotation_beta = rotation_vector[alpha_size:]

        rotated_coeff_alpha = _rotate_mo(mf.mo_coeff[0], mf.mo_occ[0], rotation_alpha)
        rotated_coeff_beta = _rotate_mo(mf.mo_coeff[1], mf.mo_occ[1], rotation_beta)
