...
def azure_quantum_load():
    """Load the Azure Quantum circuit executor into QDK/Chemistry."""
    Logger.trace_entering()

    from qdk_chemistry.algorithms import register  # noqa: PLC0415
    from qdk_chemistry.plugins.azure_quantum.circuit_executor import AzureQuantumBackend  # noqa: PLC0415

    register(lambda: AzureQuantumBackend())
    Logger.debug(
        f"Azure Quantum plugins loaded: [{AzureQuantumBackend().type_name()}: {AzureQuantumBackend().name()}]."
    )
