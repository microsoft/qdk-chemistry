...
def get_summary(
    ...
    # Try JSON array (e.g. Pareto frontier from qdk_qre_v3)
    if filename.endswith(".json"):
        try:
            with open(filename, encoding="utf-8") as f:
                raw = json.load(f)
            if isinstance(raw, list) and raw and isinstance(raw[0], dict):
                # Try to deserialise as list of ResourceEstimatorData
                from qdk_chemistry.data.resource_estimator_data import ResourceEstimatorData  # noqa: PLC0415

                items = [ResourceEstimatorData.from_json(entry) for entry in raw]
                summaries = [item.get_summary() for item in items]
                result = {
                    "data_type": "list[ResourceEstimatorData]",
                    "count": len(items),
                    "summary": summaries[0],
                    "pareto_range": {
                        "min_physical_qubits": min(it.physical_counts.physical_qubits for it in items),
                        "max_physical_qubits": max(it.physical_counts.physical_qubits for it in items),
                        "min_runtime_ns": min(it.physical_counts.runtime for it in items),
                        "max_runtime_ns": max(it.physical_counts.runtime for it in items),
                    },
                    "points": [
                        {
                            "physical_qubits": it.physical_counts.physical_qubits,
                            "runtime_ns": it.physical_counts.runtime,
                            "runtime_unit": it.physical_counts.runtime_unit,
                            "error": it.error,
                        }
                        for it in items
                    ],
                }
                # Surface assumptions from config
                first = items[0] if items else None
                if first and first.config:
                    cfg = first.config
                    assumptions = {
                        "qubit_model": cfg.qubit_model,
                        "qec_scheme": cfg.qec_scheme,
                        "error_budget": cfg.error_budget,
                    }
                    if cfg.gate_time_ns:
                        assumptions["gate_time_ns"] = cfg.gate_time_ns
                    if cfg.measurement_time_ns:
                        assumptions["measurement_time_ns"] = cfg.measurement_time_ns
                    if cfg.factory:
                        assumptions["factory"] = cfg.factory
...
def run_resource_estimation(
    ...
    This tool runs a quantum resource estimator on a circuit to produce a detailed
    resource profile including logical counts (qubits, T-count, rotation count),
    physical counts (physical qubits, runtime, RQOPS), logical qubit properties
    (code distance, cycle time, error rate), and an error budget breakdown.
    ...
    The default estimator (``qdk_qre_v3``) uses the QRE v3 engine to explore the full
    Pareto frontier (qubits vs runtime tradeoffs).  The output file contains a
    JSON array of ResourceEstimatorData objects — one per Pareto-optimal point.
    Use ``visualize_scatter_plot`` to plot the frontier.

    The ``qdk_qre_v1`` estimator uses the Q# resource estimator and
    supports circuits provided as Q# factory data or OpenQASM.  It returns
    a single estimation point rather than a Pareto frontier.

    Usage guidelines:

    - The default resource estimator can be extracted using
      the function and MCP tool `get_algorithm_default_type`.
    - The ``error_budget`` setting controls the total error budget for the
      estimation (default: 0.001). A smaller error budget yields more
      conservative (larger) physical resource estimates.
    ...
    - The current set of default settings can be obtained by using
      the function and MCP tool `get_algorithm_default_settings`.
    - ``error_budget`` (float): Total error budget for the estimation.
      Default: 0.01 (v3), 0.001 (v1). Smaller values increase physical
      resource requirements.
    - ``gate_time`` (int, default 50): Single-qubit gate time in ns (v3 only).
    - ``measurement_time`` (int, default 100): Measurement time in ns (v3 only).
    ...
    Args:
        ...
        algorithm_name (str, optional): Algorithm name to override the default estimator
        settings (Dict, optional): A dictionary of key-value pairs specifying which
            settings keys to replace with specific values (overrides defaults)
        cache (str, optional): Cache backend identifier for result caching
        remote (str, optional): Remote backend identifier for remote execution
        remote_config (dict, optional): Configuration options for the remote backend
        remote_timeout (int): Maximum seconds to wait for a remote job
    ...
    Returns:
        str: Filename where ResourceEstimatorData was saved
        OR error message if there was a problem with the workflow.

    """
    circuit_filename = _strip(circuit_filename)
    out_resource_estimator_data_filename = _strip(out_resource_estimator_data_filename)
    out_resource_estimator_data_filename, _err = _prepare_output(
        out_resource_estimator_data_filename, "ResourceEstimatorData", data.ResourceEstimatorData, overwrite=overwrite
    )
    if _err:
        return _err
    ...
    resource_estimator = algorithms.create("resource_estimator", algorithm_name)

    _apply_settings(resource_estimator, settings)

    resource_estimator_data = _run_algorithm(
        resource_estimator,
        circuit,
        cache=cache,
        remote=remote,
    ...
    # Handle list results (e.g. Pareto frontier from qdk_qre_v3)
    if isinstance(resource_estimator_data, list):
        json_array = [entry.to_json() for entry in resource_estimator_data]
        fname = out_resource_estimator_data_filename
        with open(fname, "w", encoding="utf-8") as f:
            json.dump(json_array, f, indent=2)
        return out_resource_estimator_data_filename

    save_data_object(resource_estimator_data, out_resource_estimator_data_filename)
