...
/// from which it builds W = R·B. Any block encoding that fits that shape - SOSSA,
...
namespace QDKChemistry.Utils.UnaryPhaseEstimation {
    ...
    ///
    /// Mirrors `QDKChemistry.Utils.SOSSAWalk.MakeSOSSAWalkOp`: the caller picks between the
    ...
    /// Nothing here is SOSSA-specific: it exercises the block-encoding-agnostic path with a
    /// completely different block encoding.
