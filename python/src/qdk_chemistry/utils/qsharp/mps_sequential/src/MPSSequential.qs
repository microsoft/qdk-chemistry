...
// =============================================================================
// Composable Op wrapper (allocates ancilla internally)
// =============================================================================

/// Parameters struct for MPS sequential state preparation.
...
/// Applies MPS sequential state preparation on the system qubit array.
/// Ancilla qubits are allocated internally (they start and end in |0⟩).
operation ApplyMPSSequential(
    params : MPSSequentialParams,
    qubits : Qubit[]
) : Unit {
...
/// Returns a Qubit[] => Unit callable for MPS sequential state preparation.
function MakeMPSSequentialOp(params : MPSSequentialParams) : Qubit[] => Unit {
    ApplyMPSSequential(params, _)
}

// =============================================================================
// Grouped composable Op wrapper (per-shape caching for fast resource estimation)
// =============================================================================

/// Parameters struct for grouped MPS sequential state preparation.
...
/// Applies grouped MPS sequential state preparation on the system qubit array.
/// Ancilla qubits are allocated internally (they start and end in |0⟩).
/// Uses per-shape estimate caching for accurate fast resource estimation.
operation ApplyMPSSequentialGrouped(
    params : MPSSequentialGroupedParams,
    qubits : Qubit[]
) : Unit {
    use ancilla = Qubit[params.numAncillaQubits];
    ...
    // Single shared angle register for QROM-loaded angles
