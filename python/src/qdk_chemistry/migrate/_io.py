...
def subgroup_to_json(group: h5py.Group, data_class, type_token: str, *, at_root: bool = False) -> dict:
    ...
    internal layout. ``at_root`` selects the unwrapped file layout used by
    Orbitals and symmetry-blocked tensors; BasisSet uses a named group.
    """
    with tempfile.TemporaryDirectory() as tmp:
        h5_path = Path(tmp) / f"sub.{type_token}.h5"
        with h5py.File(h5_path, "w") as dst:
            if at_root:
                dst.attrs.update(group.attrs)
                for name in group:
                    dst.copy(group[name], name)
            else:
