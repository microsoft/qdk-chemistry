...
def _typed_obj_to_dict(obj: Any) -> dict[str, Any]:
    """Serialize a __slots__-based object to a dict."""
    return {s: getattr(obj, s) for s in obj.__slots__}


def _typed_obj_to_hdf5(obj: Any, group: h5py.Group) -> None:
...
class ResourceEstimatorData(DataClass):
    ...
    def _hash_update(self, h) -> None:
        ...
        h.update(self.estimator.encode())
        h.update(_json.dumps(self.to_json(), sort_keys=True).encode())
    ...
    def get_summary(self) -> str:
        ...
        lq = self.logical_qubit
        eb = self.error_budget
        lines = [
            f"Resource Estimator Data (estimator: {self.estimator})",
            f"  Status: {self.status}",
            f"  Total error: {self.error}",
            "",
            "  Logical Counts:",
            f"    Logical qubits: {lc.num_qubits}",
            f"    T-count: {lc.t_count}",
            f"    Rotation count: {lc.rotation_count}",
            f"    Rotation depth: {lc.rotation_depth}",
            f"    CCZ count: {lc.ccz_count}",
            f"    CCiX count: {lc.ccix_count}",
            f"    Measurement count: {lc.measurement_count}",
            "",
            "  Physical Counts:",
            f"    Physical qubits: {pc.physical_qubits}",
            f"    Algorithm qubits: {pc.algorithm_qubits}",
            f"    Factory qubits: {pc.factory_qubits}",
            f"    Runtime: {pc.runtime} {pc.runtime_unit}",
            f"    RQOPS: {pc.rqops}",
            f"    Algorithmic logical depth: {pc.algorithmic_logical_depth}",
            f"    Logical depth: {pc.logical_depth}",
            "",
            "  Logical Qubit:",
            f"    Code distance: {lq.code_distance}",
            f"    Logical cycle time: {lq.logical_cycle_time} ns",
            f"    Logical error rate: {lq.logical_error_rate:.2e}",
            f"    Physical qubits per logical qubit: {lq.physical_qubits}",
            "",
            "  Error Budget:",
            f"    Logical: {eb.logical:.2e}",
            f"    Rotations: {eb.rotations:.2e}",
            f"    T-states: {eb.tstates:.2e}",
        ]
        if self.config is not None:
            cfg = self.config
            lines.append("")
            lines.append("  Estimation Config:")
            lines.append(f"    Qubit model: {cfg.qubit_model}")
            lines.append(f"    QEC scheme: {cfg.qec_scheme}")
            lines.append(f"    Error budget: {cfg.error_budget}")
            if cfg.gate_time_ns:
                lines.append(f"    Gate time: {cfg.gate_time_ns} ns")
            if cfg.measurement_time_ns:
                lines.append(f"    Measurement time: {cfg.measurement_time_ns} ns")
            if cfg.factory:
                lines.append(f"    Factory: {cfg.factory}")
            if cfg.description:
                lines.append(f"    Description: {cfg.description}")
        if self.circuit_counts is not None:
            cc = self.circuit_counts
            lines.append("")
            lines.append("  Circuit Counts:")
            lines.append(f"    Depth: {cc.depth}")
            lines.append(f"    Total gates: {cc.num_gates}")
            lines.append(f"    Single-qubit Clifford: {cc.num_single_qubit_clifford}")
            lines.append(f"    Two-qubit Clifford: {cc.num_two_qubit_clifford}")
            lines.append(f"    Non-Clifford: {cc.num_non_clifford}")
        return "\n".join(lines)
