...
This module provides the :class:`MajoranaMapping` data class, which describes a
fermion-to-qubit encoding. It stores a 2N-entry table mapping each Majorana
operator gamma_k to a Pauli word; the bilinear ``i*gamma_j*gamma_k`` is the
unified primitive and is computed on demand from the table.
...
_CHAR_TO_OP = {"I": 0, "X": 1, "Y": 2, "Z": 3}
_OP_TO_CHAR = {1: "X", 2: "Y", 3: "Z"}

SparsePauliWord = list[tuple[int, int]]


def _dense_le_to_sparse(label: str) -> SparsePauliWord:
    """Convert a dense little-endian Pauli string to a sparse Pauli word."""
    n = len(label)
    word: SparsePauliWord = []
    for i, char in enumerate(label):
        try:
            op = _CHAR_TO_OP[char.upper()]
        except KeyError:
            raise ValueError(f"Invalid Pauli character {char!r}; expected I, X, Y, or Z") from None
        if op:
            word.append((n - 1 - i, op))
    word.sort()
    return word


def _sparse_to_dense_le(word: SparsePauliWord, num_qubits: int) -> str:
    """Convert a sparse Pauli word to a dense little-endian string."""
    chars = ["I"] * num_qubits
    for qubit, op in word:
        if qubit < num_qubits:
            chars[num_qubits - 1 - qubit] = _OP_TO_CHAR[op]
    return "".join(chars)
...
class MajoranaMapping(DataClass):
    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            ValueError: If the table is invalid (empty, odd size, or bad characters).
    ...
    def is_majorana_atomic(self) -> bool:
        ...
        When ``True``, :py:meth:`majorana` is available; otherwise only
        :py:meth:`bilinear` is.
        """
    ...
    def bilinear(self, j: int, k: int) -> tuple[complex, str]:
        ...
        Returns ``(coeff, pauli_str)`` such that ``coeff * pauli_str`` equals
        ``i*gamma_j*gamma_k`` in the encoded qubit representation. Defined for
        ...
        """
        coeff, word = self._core.bilinear(j, k)
        return coeff, _sparse_to_dense_le(word, self._num_qubits)
    ...
    def from_hdf5(cls, group: h5py.Group) -> MajoranaMapping:
        ...
        return cls(table=table, name=name, tapering=tapering)
