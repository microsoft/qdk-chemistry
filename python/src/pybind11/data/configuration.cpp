...
void bind_configuration(pybind11::module &data) {
  ...
  py::class_<Configuration, DataClass, py::smart_holder> configuration(
      ...
      Represents a molecular electronic configuration.
        ...
        This class efficiently stores the occupation pattern of molecular orbitals
        using a compact representation where each orbital can be in one of four states:
  ...
  configuration.def(py::init<const std::string &>(),
        ...
        ----------
        str : str
            String representation of the configuration where:
            '0' = unoccupied orbital
            '1' = alpha-occupied orbital
            '2' = beta-occupied orbital
            '3' = doubly-occupied orbital
  ...
  configuration.def("__eq__", &Configuration::operator==,
        ...
        ----------
        other : Configuration
            Another configuration to compare with
  ...
  configuration.def("__ne__", &Configuration::operator!=,
        ...
        -------
        bool
            True if configurations are different, False otherwise
  ...
  configuration.def(
      ...
      R"(
        ...
        -------
        str
            String representation of the Configuration as a orbital occupation string
        )");
}
