...
void bind_lattice_graph(pybind11::module &m) {
  ...
  py::class_<NeighborConnection>(m, "NeighborConnection",
                                 "Physical connection in a lattice geometry.")
      .def_property_readonly(
...
Returns:
  ...
  lattice_graph.def_property_readonly(
      "positions", [](const LatticeGraph &self) { return self.positions(); },
      R"(
Cartesian positions generated by the lattice factory.

Returns:
        numpy.ndarray | None: A copy of the ``(num_sites, 2)`` position matrix, or ``None`` when geometry is unavailable.
)");
...
Raises:
  ...
  lattice_graph.def("neighbor_connections", &LatticeGraph::neighbor_connections,
                    R"(
Return physical connections classified by geometric shell and bond axis.

Distinct periodic images are retained even when they connect the same pair of
finite-lattice sites. Semantic ``flavor`` values are optional.

Args:
    shells (list[int]): One-based shell indices.
    tolerance (float, optional): Relative tolerance used to group distances and axes. Defaults to 1e-9.

Returns:
    list[NeighborConnection]: Deterministically ordered physical connections.
)",
                    py::arg("shells"), py::arg("tolerance") = 1.0e-9);
  lattice_graph.def("with_bond_flavors", &LatticeGraph::with_bond_flavors,
                    R"(
Return a copy with semantic labels assigned to selected shell-axis classes.

Args:
    definitions (list[BondFlavorDefinition]): Shell-axis flavor definitions.
    tolerance (float, optional): Axis comparison tolerance. Defaults to 1e-9.

Returns:
    LatticeGraph: Graph with unchanged topology and geometry plus flavor metadata.
)",
                    py::arg("definitions"), py::arg("tolerance") = 1.0e-9);
  lattice_graph.def_property_readonly(
      "bond_flavor_definitions", &LatticeGraph::bond_flavor_definitions,
      R"(
Semantic flavor definitions attached to geometric shell-axis classes.

Returns:
    list[BondFlavorDefinition]: Flavor definitions, or an empty list when none are defined.
)",
