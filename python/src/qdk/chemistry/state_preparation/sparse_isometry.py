...
All implementations inherit from the base StatePrep class and follow the
factory pattern for consistent instantiation. The sparse isometry algorithms
are particularly well-suited for quantum chemistry applications where
electronic structure wavefunctions often have a small number of dominant
determinants.
...
from qdk.chemistry.plugins.qiskit._interop.circuit import CircuitInfo
from qdk.chemistry.utils.bitstring import bitstrings_to_binary_matrix

from .base import StatePrep, StatePrepAlgorithm, prepare_single_reference_state

_LOGGER = logging.getLogger(__name__)


class SparseIsometryGF2XStatePrep(StatePrep):
    ...
    algorithm = StatePrepAlgorithm.SPARSE_ISOMETRY_GF2X
    ...
    def __init__(
        ...
        summary_filename: str = "sparse_isometry_gf2x_summary.state_preparation.json",
        output_dir: str = "sparse_isometry_gf2x_output",
    ):
        """Initialize the sparse isometry GF2+X state preparation.

        Args:
            wavefunction: Wavefunction to prepare state from
            max_dets: Maximum number of determinants to include (default: include all)
            amplitude_threshold: Amplitude absolute value threshold for including determinants (default: 0.0)
            save_outputs: Whether to save circuit diagrams and summary files
            summary_filename: Filename for the summary file (only used if save_outputs=True)
            output_dir: Directory to save output files (only used if save_outputs=True)
    ...
    def create_circuit_qasm(self):
        """Create a quantum circuit that prepares the state using sparse isometry with GF2+X elimination.

        Returns:
            A QASM string representation of the quantum circuit.

        """
        circuit = self._create_sparse_isometry_circuit()
        return qasm3.dumps(circuit)

    def _create_sparse_isometry_circuit(self) -> QuantumCircuit:
        """Create a quantum circuit that prepares the state using sparse isometry with GF2+X elimination.

        Returns:
            A quantum circuit implementing the sparse isometry GF2+X state preparation

        Raises:
            ValueError: If no determinants remain after filtering

        """
        # Step 1: Filter terms based on amplitude threshold and max_dets
        coeffs, bitstrings = self._filter_terms(renormalize=False)
        ...
        # Step 2: Convert bitstrings to binary matrix
        bitstring_matrix = bitstrings_to_binary_matrix(bitstrings)

        # Step 3: Apply enhanced GF2+X elimination
        ...
        cnot_ops = [op for op in gf2x_operation_results.operations if op[0] == "cnot"]
        x_ops = [op for op in gf2x_operation_results.operations if op[0] == "x"]
        _LOGGER.debug(f"CNOT operations: {len(cnot_ops)}")
        _LOGGER.debug(f"X operations: {len(x_ops)}")

        # Step 4: Create quantum circuit
        qc = QuantumCircuit(
            n_qubits,
            name=f"sparse_isometry_gf2x_{self.max_dets}_dets",
        )

        # Step 5: Create statevector for the reduced matrix
        if gf2x_operation_results.rank > 0:
            ...
            for i, amp in enumerate(statevector_data):
                if abs(amp) > self.amplitude_threshold:  # Only show non-negligible amplitudes
            ...
            # Step 6: Apply dense state preparation on reduced space
            _LOGGER.debug(f"Target indices are {gf2x_operation_results.row_map}")
            qc.append(StatePreparation(statevector, normalize=False), gf2x_operation_results.row_map)
        else:
        ...
        # Step 7: Apply recorded operations in reverse order to expand back to full space.
        ...
        _LOGGER.info(f"Final circuit: {qc.num_qubits} qubits, depth {qc.depth()}, {qc.size()} gates")
        ...
        if self.save_outputs:
            self._save_outputs(
                ...
                original_matrix=bitstring_matrix,
                elimination_results=gf2x_operation_results,
            )

        return qc

    def _collect_operations_summary(
        self,
        circuit: QuantumCircuit,
        original_matrix: np.ndarray,
        elimination_results: "GF2XEliminationResult",
    ) -> dict:
        """Collect summary information as structured data.

        Args:
            circuit: The quantum circuit
            original_matrix: Original bitstring matrix
            elimination_results: Result of the GF2+X elimination process

        Returns:
            Structured summary data as a GF2XSummary dictionary

        """
        return {
            "metadata": {
                "num_orbitals": self.num_orbitals,
                "num_determinants": len(self.wavefunction.get_active_determinants()),
                "max_dets": self.max_dets,
                "amplitude_threshold": self.amplitude_threshold,
            },
            "matrix_operations": {
                "original_shape": original_matrix.shape,
                "original_rank": int(np.linalg.matrix_rank(original_matrix)),
                "reduced_shape": elimination_results.reduced_matrix.shape,
                "reduced_rank": elimination_results.rank,
                "row_map": elimination_results.row_map,
                "operation_order": elimination_results.operations,
                "total_operations": len(elimination_results.operations),
                "cnot_operations": len([op for op in elimination_results.operations if op[0] == "cnot"]),
                "x_operations": len([op for op in elimination_results.operations if op[0] == "x"]),
            },
            "untranspiled_circuit_summary": CircuitInfo(circuit=circuit).summary(),
        }
    ...
    def _save_outputs(
        ...
        """Save matrix operation summary information for sparse isometry state preparation algorithms.
        ...
        summary_data = self._collect_operations_summary(
            circuit=circuit,
        ...
        try:
            ...
            _LOGGER.warning(f"Failed to save summary file to {summary_file}: {e}")
