...
from qdk.chemistry.algorithms import ScfSolver, register
...
from qdk.chemistry.plugins.pyscf.utils import orbitals_to_scf, pyscf_mol_to_qdk_basis, structure_to_pyscf_atom_labels
    ...
    def _run_impl(
        ...
        * DFT variants (RKS/UKS/ROKS) for any other method string, treating it as an exchange-correlation functional
        ...
        if initial_guess is not None and hasattr(initial_guess, "get_coefficients"):
            ...
            norb = initial_guess.get_num_mos()
            nocc_a = mol.nelec[0]  # Number of alpha electrons
            nocc_b = mol.nelec[1]  # Number of beta electrons

            occ_alpha = np.array([1.0 if i < nocc_a else 0.0 for i in range(norb)])
            occ_beta = np.array([1.0 if i < nocc_b else 0.0 for i in range(norb)])
