"""Test for circuit executor in QDK/Chemistry qiskit interop."""

# --------------------------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See LICENSE.txt in the project root for license information.
# --------------------------------------------------------------------------------------------

import pytest

from qdk_chemistry.data import Circuit, QuantumErrorProfile
from qdk_chemistry.plugins.qiskit import QDK_CHEMISTRY_HAS_QISKIT_AER

if QDK_CHEMISTRY_HAS_QISKIT_AER:
    from qdk_chemistry.plugins.qiskit.circuit_executor import QiskitAerSimulator


pytestmark = pytest.mark.skipif(not QDK_CHEMISTRY_HAS_QISKIT_AER, reason="Qiskit Aer not available")


@pytest.fixture
def test_circuit_1() -> Circuit:
    """Create a test circuit."""
    return Circuit(
        qasm="""
        OPENQASM 3.0;
        include "stdgates.inc";
        qubit[2] q;
        bit[2] c;
        h q[0];
        cx q[0], q[1];
        c[0] = measure q[0];
        c[1] = measure q[1];
        """,
    )


@pytest.fixture
def test_circuit_2() -> Circuit:
    """Create a test circuit with fixed bitstring outcomes."""
    return Circuit(
        qasm="""
        OPENQASM 3.0;
        include "stdgates.inc";
        qubit[2] q;
        bit[2] c;
        x q[0];
        c[0] = measure q[0];
        c[1] = measure q[1];
        """,
    )


class TestQiskitAerCircuitExecutor:
    """Test suite for Qiskit Aer circuit executor."""

    def test_initialization(self):
        """Test initialization of the executor."""
        executor = QiskitAerSimulator()
        assert executor.settings().get("seed") == 42
        assert executor.settings().get("method") == "statevector"

    def test_circuit_executor_qiskit_aer(self, test_circuit_1: Circuit, test_circuit_2: Circuit):
        """Test the Qiskit Aer circuit executor."""
        executor = QiskitAerSimulator()

        # Test circuit 1 (Bell state): only "00" and "11" outcomes with ~50/50 split
        result_1 = executor.run(test_circuit_1, shots=1000)
        counts_1 = result_1.bitstring_counts
        total = sum(counts_1.values())
        assert counts_1.get("01", 0) + counts_1.get("10", 0) == 0, "Bell state must not produce 01 or 10"
        p_00 = counts_1.get("00", 0) / total
        assert 0.35 < p_00 < 0.65, f"Bell state P(00)={p_00:.3f}, expected ~0.5"

        # Test circuit 2, which will always return "10" outcome
        result_2 = executor.run(test_circuit_2, shots=10)
        counts_2 = result_2.bitstring_counts
        raw_data = result_2.get_executor_metadata()
        assert counts_2.get("01", 0) == 10  # "10" in qubit order is "01" in bitstring order
        assert counts_2.get("00", 0) + counts_2.get("10", 0) + counts_2.get("11", 0) == 0
        assert raw_data.backend_name == "aer_simulator_statevector"

    def test_circuit_executor_with_error_profile(
        self, test_circuit_1: Circuit, simple_error_profile: QuantumErrorProfile
    ):
        """Test the Qiskit Aer circuit executor with an error profile."""
        executor = QiskitAerSimulator()
        result = executor.run(test_circuit_1, shots=100, noise=simple_error_profile)
        counts = result.bitstring_counts
        assert counts.get("00", 0) > 0
        assert counts.get("11", 0) > 0
        assert counts.get("01", 0) + counts.get("10", 0) > 0  # Expect some errors to occur
