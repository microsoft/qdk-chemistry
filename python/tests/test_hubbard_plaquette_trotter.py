...
def _resolve_divisions(
    *,
    ...
    return builder._resolve_num_divisions(_lattice_operator(side, side), time)
...
class TestHammingWeightPhasing:
    ...
    def test_unbounded_batches_merge_the_interaction_families(self):
        ...
        """
        side = 4
        operator = _lattice_operator(side, side)
        shapes = {}
...
class TestPlaquetteTrotterPhaseEstimation:
    ...
    def test_qpe_recovers_the_exact_eigenvalue(self):
        ...
        result = iqpe.run(
            qubit_hamiltonian=_lattice_operator(self._SIDE, self._SIDE),
...
def _logical_counts(size: int, *, max_batch: int) -> dict:
    ...
    unitary = create(
        ...
        t=_HOPPING,
        U=_INTERACTION,
        epsilon=-_INTERACTION / 2.0,
        max_batch=max_batch,
        weight_threshold=_WEIGHT_THRESHOLD,
    ).run(_lattice_operator(size, size))
    circuit = create(
