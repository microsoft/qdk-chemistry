...
class TestValenceParameters:
    def test_compute_valence_space(self):
        symbols = ["O", "H", "H"]
        coords = np.array(
        ...
        (num_active_electrons, num_active_orbitals) = compute_valence_space(wavefunction_sd, 0)
        assert num_active_electrons == 8  # number of valence electrons
        assert num_active_orbitals == 6  # number of valence orbitals

    def test_compute_valence_space_truncated(self):
        symbols = ["O", "H", "H"]
        coords = np.array(
            [[0.000000, 0.000000, 0.000000], [0.757000, 0.586000, 0.000000], [-0.757000, 0.586000, 0.000000]]
        )
        ...
        (num_active_electrons, num_active_orbitals) = compute_valence_space(wavefun_truncated, 0)
        assert num_active_electrons == 8  # number of valence electrons
        assert num_active_orbitals == 4  # number of valence orbitals

    def test_compute_valence_space_helium(self):
        ...
        (num_active_electrons, num_active_orbitals) = compute_valence_space(wavefun_he, 0)

        assert num_active_electrons == 2  # number of valence electrons
        assert num_active_orbitals == 1  # number of valence orbitals

    def test_compute_valence_space_oxygen_hydrogen(self):
        ...
        (num_active_electrons, num_active_orbitals) = compute_valence_space(wavefun_oh, 0)

        assert num_active_electrons == 7  # number of valence electrons
        assert num_active_orbitals == 5  # number of valence orbitals

    def test_compute_valence_space_positive_oxygen_hydrogen(self):
        symbols = ["O", "H"]
        coords = np.array([[0.000000, 0.000000, 0.000000], [0.757000, 0.586000, 0.000000]])
        oh_molecule = Structure(symbols, coords)
        ...
        # Test with +1 charge
        (num_active_electrons, num_active_orbitals) = compute_valence_space(wavefun_ohp, 1)

        assert num_active_electrons == 6  # number of valence electrons
        assert num_active_orbitals == 5  # number of valence orbitals

    def test_compute_valence_space_negative_oxygen_hydrogen(self):
        symbols = ["O", "H"]
        coords = np.array([[0.000000, 0.000000, 0.000000], [0.757000, 0.586000, 0.000000]])
        oh_molecule = Structure(symbols, coords)
        ...
        # Test with -1 charge
        (num_active_electrons, num_active_orbitals) = compute_valence_space(wavefun_ohn, -1)
