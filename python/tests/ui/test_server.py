...
get_active_space_indices = _make_unwrapped(_server.get_active_space_indices)
get_ansatz = _make_unwrapped(_server.get_ansatz)
get_orbitals_from_input = _make_unwrapped(_server.get_orbitals_from_input)
get_summary = _make_unwrapped(_server.get_summary)
get_top_determinants = _make_unwrapped(_server.get_top_determinants)
...
class TestHelperFunctions:
    ...
    @pytest.mark.parametrize("algorithm_type", ["energy_estimator", "phase_estimation", "geometry_optimizer"])
    def test_get_algorithm_default_settings_serializes_algorithm_ref(self, algorithm_type):
        """Test default settings with nested AlgorithmRef values are JSON serializable."""
        settings = get_algorithm_default_settings(algorithm_type)
        json.dumps(settings)

        assert "_descriptions" not in settings
        if algorithm_type == "geometry_optimizer":
            derivative_calculator = settings["derivative_calculator"]
            assert derivative_calculator["__type__"] == "algorithm_ref"
            assert derivative_calculator["algorithm_type"] == "nuclear_derivative_calculator"
            assert derivative_calculator["algorithm_name"] == "qdk"
            json.dumps(derivative_calculator["settings"])

    def test_algorithm_default_settings_round_trip_nested_algorithm_ref(self):
        """MCP-serialized nested algorithm settings can be reapplied unchanged."""
        settings = get_algorithm_default_settings("geometry_optimizer")
        settings["derivative_calculator"]["settings"]["suppress_child_algorithm_logging"] = False
        optimizer = algorithms.create("geometry_optimizer")

        _server._apply_settings(optimizer, settings)

        derivative_calculator = optimizer.settings().get("derivative_calculator")
        assert derivative_calculator.algorithm_type == "nuclear_derivative_calculator"
        assert derivative_calculator.algorithm_name == settings["derivative_calculator"]["algorithm_name"]
        assert derivative_calculator.settings.get("suppress_child_algorithm_logging") is False
...
class TestAlgorithmFunctions:
    ...
    def test_get_top_determinants(self, wavefunction_4e4o, temp_project_dir):
        """Return ranked determinant coefficients and normalized weights."""
        tool_listing = _unwrap(_server.list_tools(category="data_inspection"))
        assert "get_top_determinants" in tool_listing["categories"]["data_inspection"]

        project_path = temp_project_dir / "test_top_determinants"
        project_path.mkdir(exist_ok=True)
        wavefunction_4e4o.to_json_file(project_path / "input.wavefunction.json")

        result = get_top_determinants(
            project_name="test_top_determinants",
            wavefunction_filename="input.wavefunction.json",
            max_determinants=2,
        )

        assert result["total_determinants"] == 2
        assert result["returned_determinants"] == 2
        assert result["norm"] == pytest.approx(1.0)
        assert [item["configuration"] for item in result["determinants"]] == ["2200", "2020"]
        assert result["determinants"][0]["coefficient_real"] == pytest.approx(-0.9837947571031265)
        assert result["determinants"][0]["coefficient_imaginary"] == pytest.approx(0.0)
        assert result["determinants"][0]["weight"] > result["determinants"][1]["weight"]
        assert result["determinants"][-1]["cumulative_weight"] == pytest.approx(1.0)
...
class TestPhaseEstimationFunctions:
    ...
    def test_run_phase_estimation_iterative(self, h2_structure, temp_project_dir):
        ...
        result = run_phase_estimation(
            ...
            settings={
                "qpe_circuit_builder": {
                    "num_bits": 4,
                    "unitary_builder": {"algorithm_name": "trotter", "time": 0.1},
                },
                "shots_per_bit": 10,
            },
        )
    ...
    def test_run_phase_estimation_with_invalid_settings(
        ...
        result = run_phase_estimation(
            ...
            qubit_hamiltonian_filename=qubit_ham_filename,
            out_qpe_result_filename="qpe.qpe_result.json",
            settings={"qpe_circuit_builder": {"unitary_builder": {"time": 0.1}}},  # Missing num_bits
        )
...
class TestNonDefaultAlgorithms:
    ...
    def test_active_space_selector_autocas(self, h2_structure, temp_project_dir):
        ...
        summary = get_summary(project_name="test_autocas", filename="autocas_active.wavefunction.json")
        assert isinstance(summary, dict)
        assert summary["data_type"] == "Wavefunction"

        indices = get_active_space_indices(
            project_name="test_autocas", input_filename="autocas_active.wavefunction.json"
        )
        assert isinstance(indices, dict)
        assert "active" in indices
