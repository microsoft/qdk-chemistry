...
class TestTrotter:
    ...
    def test_fourth_order_preserves_angle_rounding(self, sparse):
        """Calibration fingerprints retain each input representation's multiplication order."""
        ...
        expected = [
            ExponentiatedPauliTerm(
                {0: "X" if i % 2 == 0 else "Z"},
                float(coefficients[i % 2]) * dt * fraction if sparse else float(coefficients[i % 2]) * (dt * fraction),
            )
    ...
    def test_multiple_trotter_steps_second_order(self):
        ...
        builder = Trotter(num_divisions=4, order=2, time=0.2)
        unitary = builder.run(hamiltonian)
    ...
    def test_multiple_trotter_steps_fourth_order(self):
        ...
        builder = Trotter(num_divisions=4, order=4, time=0.2)
        unitary = builder.run(hamiltonian)
    ...
    def test_multiple_trotter_steps_sixth_order(self):
        ...
        builder = Trotter(num_divisions=4, order=6, time=0.2)
        unitary = builder.run(hamiltonian)
    ...
    def test_trotter_x_z_example_higher_order(self):
        ...
        t = 0.1
        builder = Trotter(num_divisions=1, order=4, time=t)
        unitary = builder.run(hamiltonian)
        container = unitary.get_container()
...
class TestTrotterAccuracyAware:
    ...
    def test_commutator_tighter_than_naive(self):
        ...
        builder_naive = Trotter(target_accuracy=eps, error_bound="naive", time=time)
        n_comm = builder_comm.run(hamiltonian).get_container().step_reps
        n_naive = builder_naive.run(hamiltonian).get_container().step_reps
    ...
    def test_commutator_tighter_than_naive_second_order(self):
        ...
        builder_naive = Trotter(target_accuracy=eps, error_bound="naive", order=2, time=time)
        n_comm = builder_comm.run(hamiltonian).get_container().step_reps
        n_naive = builder_naive.run(hamiltonian).get_container().step_reps
        assert n_comm <= n_naive
    ...
    def test_commutator_tighter_than_naive_higher_order(self):
        ...
        builder_naive = Trotter(target_accuracy=eps, error_bound="naive", order=4, time=time)
        n_comm = builder_comm.run(hamiltonian).get_container().step_reps
        n_naive = builder_naive.run(hamiltonian).get_container().step_reps
        assert n_comm <= n_naive
    ...
    def test_angle_scaling_with_auto_steps_higher_order(self):
        ...
        t = 1.0
        builder = Trotter(target_accuracy=0.01, order=4, time=t)
...
class TestPartitionGrouping:
    ...
    @pytest.mark.parametrize("sparse", [False, True])
    def test_empty_layers_preserve_feature_group_order(self, sparse):
        """Sparse groups count empty layers; dense groups discard them before stable sorting."""
        ...
        expected = [{1: "Y"}, {2: "Z"}, {0: "X"}] if sparse else [{0: "X"}, {1: "Y"}, {2: "Z"}]
        assert [term.pauli_term for term in terms] == expected
