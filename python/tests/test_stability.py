...
class TestStabilityResultIO:
    ...
    def test_stability_result_json_serialization(self):
        ...
        # On Windows the default (delete=True) keeps an exclusive lock on the file. We delete the file manually in
        # `finally` via Path.unlink(). This pattern is used throughout the test suite.
        # NOTE: Python 3.12+ supports `delete=False, delete_on_close=True` which would avoid the manual unlink().
