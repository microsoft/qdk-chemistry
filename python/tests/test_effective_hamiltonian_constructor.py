...
class _TestEffectiveHamiltonianConstructor(EffectiveHamiltonianConstructor):
    """Minimal EffectiveHamiltonianConstructor subclass used as a test double."""
    ...
    def name(self):
        """Return the registered name of this test constructor."""
        return "test"

    def _run_impl(self, reference, hamiltonian, p_indices):
        """Assert the forwarded reference and P-space, then echo the Hamiltonian."""
        assert reference is self.expected_reference
        assert p_indices is self.expected_indices
        return hamiltonian
...
def test_run_forwards_p_indices(p_indices):
    """The fixed run signature forwards restricted and unrestricted P-spaces."""
    reference = create_test_wavefunction()
    constructor = _TestEffectiveHamiltonianConstructor(reference, p_indices)

    assert constructor.run(reference, None, p_indices) is None
...
def test_custom_constructor_registry_round_trip():
    ...
    registry.register(lambda: _TestEffectiveHamiltonianConstructor(reference, p_indices))
    try:
