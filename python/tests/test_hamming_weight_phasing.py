...
class TestHammingWeightSchedule:
    ...
    def test_uses_the_optimal_number_of_adders(self, count):
    ...
    def test_arithmetic_costs_one_toffoli_per_adder(self, count):
...
class TestComputeHammingWeight:
    ...
    def test_counts_every_basis_state(self, count):
        """Exhaustive over all 2^m inputs: the register must hold the true weight."""
        sizes = get_qsharp_context().run(
        ...
        for value in range(2**count):
            ...
            results = get_qsharp_context().run(source, 1)[0]
...
class TestHammingWeightPhase:
    ...
    def test_matches_the_product_of_single_rotations(self, count, angle):
    ...
    def test_keeps_the_identity_phase_under_a_control(self, count):
    ...
    def test_control_falls_only_on_the_rotations(self):
...
class TestHammingWeightPhaseTerms:
    ...
    @pytest.mark.parametrize("angle", [0.31, -1.2])
    def test_matches_the_individual_exponentials(self, name, angle):
...
class TestBatchSegments:
    ...
    def test_segments(self, ids, expected):
        """Distinct identifiers must not merge, and zeros must stay singletons."""
        got = get_qsharp_context().run(
            f"{{ import QDKChemistry.Utils.HammingWeightPhasing.*; BatchSegments({ids!s}) }}", 1
        )[0]
