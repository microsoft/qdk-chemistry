...
The ``Structure`` class represents a molecular structure, i.e. the coordinates of its atoms.

``Structure`` objects can be constructed manually, or via deserialization from a file.
QDK/Chemistry supports multiple serialization formats for ``Structure`` objects, including the standard `XYZ file format <https://en.wikipedia.org/wiki/XYZ_file_format>`_, as well as QDK/Chemistry-specific JSON and HDF5 serialization schemes.
...
Once a ``Structure`` is created, an :term:`SCF` calculation can be performed to produce an initial ``Wavefunction`` as well as an :term:`SCF` energy.
QDK/Chemistry performs :term:`SCF` calculations via instantiations of the ``ScfSolver`` algorithm.
...
The inputs for an :term:`SCF` calculation are a ``Structure`` object, the charge and multiplicity of the molecular system, and information about the single-particle basis to be used.
Optionally, ``Settings`` specific to the particular ``ScfSolver`` can be configured by accessing the ``settings()`` method.
...
:term:`MC` calculations are performed via instantiations of the ``MultiConfigurationCalculator`` algorithm.
...
For large active spaces, the multi-configuration :doc:`Wavefunction <comprehensive/data/wavefunction>` may contain thousands or millions of configurations, but often only a small subset contributes significantly to the overall state.
...
This truncation is characterized by computing the overlap between the truncated state and the full wavefunction, and by recalculating the energy of the reduced wavefunction using the ``ProjectedMultiConfigurationCalculator``.
...
First, by filtering out terms that have negligible expectation values given the sparse :doc:`wavefunction <comprehensive/data/wavefunction>`, pre-computing their classical contributions.
...
Simulation and execution on a quantum simulator
-----------------------------------------------
