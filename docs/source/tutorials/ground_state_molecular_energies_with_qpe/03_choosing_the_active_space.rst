...
This localizer first anchors each selected block to the atomic-orbital basis and then rotates those blocks to minimize the valence-space mapped Hamiltonian coefficient 1-norm :math:`\lambda=\sum_k\lvert c_k\rvert`, without changing the orbital subspace or its exact :term:`CASCI` energy.
The mapped Hamiltonian and its Pauli coefficients :math:`c_k` are introduced in :doc:`Mapping the problem to qubits <04_putting_the_problem_on_qubits>`; reducing :math:`\lambda` permits the longer unaliased evolution time used in :doc:`Iterative quantum phase estimation <06_iterative_phase_estimation>`.
