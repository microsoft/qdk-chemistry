...
This tutorial uses `quantum phase estimation <https://en.wikipedia.org/wiki/Quantum_phase_estimation_algorithm>`_ (:term:`QPE`) and the Quantum Development Kit (:term:`QDK`) Chemistry library, called QDK/Chemistry in this documentation, to estimate the ground-state electronic energy of a stretched nitrogen molecule, N\ :sub:`2`.
