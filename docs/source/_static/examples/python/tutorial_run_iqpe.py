...
def create_iqpe_circuit_builder(
    evolution_time_hartree_inverse: float,
    *,
    num_phase_bits: int = DEFAULT_NUM_PHASE_BITS,
) -> QpeCircuitBuilder:
    """Configure native IQPE with repeated first-order Trotter evolution.
    ...
    Returns:
        A native iterative circuit builder using
...
def prepare_iqpe_problem(
    *,
    ...
    # Configure one builder for circuit inspection, then preserve the same
    # settings in a serializable reference used by the IQPE algorithm.
...
def run_iqpe_workflow(
    *,
    ...
    # Run repeated complete estimates
...
################################################################################
# Students can stop reading here. The definitions below package IQPE problem,
# timing, and result data for reuse across the workflow. The final two lines run
# the student-facing main() function when this file is executed.
################################################################################
