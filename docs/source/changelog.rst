...
- Robust phase estimation with the Trotter unitary builder now uses ``epsilon_rpe = target_accuracy`` and an independent dimensionless ``epsilon_unitary`` with default ``0.85``. Trotter callers must replace ``unitary_accuracy_fraction`` or explicit ``epsilon_rpe`` with ``target_accuracy`` plus an optional positive ``epsilon_unitary``. Other unitary-builder categories retain their existing routing.
