...
Additionally, the original AO three-center vectors are preserved in the container when ``store_ao_three_center_vectors`` is enabled, and can be retrieved via ``get_ao_three_center_vectors()``.
