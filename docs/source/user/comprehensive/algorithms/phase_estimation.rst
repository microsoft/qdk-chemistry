...
QubitOperator or Hamiltonian
   Either a :class:`~qdk_chemistry.data.QubitOperator` containing the Pauli-string representation of the Hamiltonian,
   or an unmapped lattice :class:`~qdk_chemistry.data.Hamiltonian`.
   A qubit Hamiltonian can be obtained from the :doc:`QubitMapper <qubit_mapper>` algorithm, constructed from a :doc:`model Hamiltonian <../model_hamiltonians>`, or built directly by the user.

   Whichever form is supplied is passed straight through to the nested unitary builder, so the two must agree.
   A lattice Hamiltonian therefore requires a lattice-aware unitary builder such as
   :ref:`plaquette <plaquette-builder>`; pairing it with a qubit-only builder raises a :class:`TypeError`
   naming both sides of the mismatch.
...
.. note::
   ...
   The state preparation circuit and Hamiltonian must be compatible — they should use the same qubit encoding and be derived from the same underlying system.
