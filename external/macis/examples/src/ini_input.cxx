...
void INIFile::parse() {
  ...
  while (not inFile_->eof()) {
    ...
    bool sectionLine = lBrckPos == firstNonSpace and rBrckPos == lastNonSpace;

    // Determine if this is a line that contains a data field
    bool dataLine = eqPos != std::string::npos or colPos != std::string::npos;
    ...
    if (sectionLine) {
      ...
      std::transform(sectionHeader.begin(), sectionHeader.end(),
                     sectionHeader.begin(),
                     [](unsigned char c) { return std::toupper(c); });

      // Create a dictionary entry for the section header
      dict_[sectionHeader] = std::unordered_map<std::string, std::string>();
    ...
    if (parseSection and dataLine) {
      line = line.substr(firstNonSpace, line.length() - firstNonSpace);
...
std::string INIFile::getData(std::string query) {
  ...
  if (hasSection != dict_.end()) {
    auto hasData = dict_[tokenPair.first].find(tokenPair.second);

    if (hasData != dict_[tokenPair.first].end())
      return dict_[tokenPair.first][tokenPair.second];
    else
      throw data_not_found(query);
  } else
    throw section_not_found(tokenPair.first);
