...
void mp2_t2(NumCanonicalOccupied _num_occupied_orbitals,
            ...
            size_t LDV, const double* eps, double* T2) {
        ...
        for (auto b = 0ul; b < num_virtual_orbitals; ++b) {
          ...
          T2[i + j * num_occupied_orbitals + a * num_occupied_orbitals2 +
             b * nocc2v] =
              ...
              // #TODO: Denominator can hit zero for near-degenerate orbitals;
              // add guard or regularization.
...
void mp2_1rdm(NumOrbital _norb, NumCanonicalOccupied _num_occupied_orbitals,
              ...
              size_t LDT, const double* V, size_t LDV, double* ORDM,
              size_t LDD) {
    ...
    for (auto j = 0ul; j < num_occupied_orbitals; ++j)
      for (auto a = 0ul; a < num_virtual_orbitals; ++a)
        for (auto b = 0ul; b < num_virtual_orbitals; ++b) {
          ...
          const double t2_ijab = T2[i + j * num_occupied_orbitals +
                                    a * num_occupied_orbitals2 + b * nocc2v];
          EMP2 += t2_ijab * (2 * V_abij - V_abji);
        }

  // #TODO: Replace std::cout with the project logger and allow disabling this
  // expensive diagnostic in production.
