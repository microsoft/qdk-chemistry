...
|------|---------------|----------------|-------------|
| SCF | `scf_solver` | `Structure` → `(energy, Wavefunction)` | Self-consistent field calculation (Hartree-Fock or DFT). Produces molecular orbital coefficients, energies, and occupations wrapped in a `Wavefunction` with a `SlaterDeterminantContainer`. |
