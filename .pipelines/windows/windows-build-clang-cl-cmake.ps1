...
$BuildDir   = "$RepoRoot\cpp\build-clang-cl" + $(if ($BuildType -ne "Release") { "-$buildTypeLower" } else { "" })
$InstallDir = "$RepoRoot\install-clang-cl"  + $(if ($BuildType -ne "Release") { "-$buildTypeLower" } else { "" })
...
if (-not $SkipCpp) {
    ...
    if (-not $SkipTests) {
        ...
        ctest --output-on-failure --verbose --timeout 400 --output-junit ctest_results.xml 2>&1 *> ctest_clang_cl.log
...
if (-not $SkipPython) {
    ...
    if (-not $SkipTests) {
        ...
        pytest -v --tb=short 2>&1 *> pytest_clang_cl.log
