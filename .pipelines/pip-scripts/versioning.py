...
def update_version_file(version_file: str, new_version_string: str) -> None:
    ...
    LOG.info(f"Updated version file at {version_file} with new version: {new_version_string}")
...
def main():
    ...
    if args.dev_tag is not None:
        ...
        if not dev_tag_is_valid:
            ...
            LOG.info(f"New version to be published on PyPI will be suffixed with the dev tag: {args.dev_tag}")
